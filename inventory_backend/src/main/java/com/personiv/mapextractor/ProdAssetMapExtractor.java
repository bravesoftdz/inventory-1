...
public class ProdAssetMapExtractor implements ResultSetExtractor<List<ProductionAsset>>{
	
	@Override
	public List<ProductionAsset> extractData(ResultSet rs) throws SQLException, DataAccessException {
		Map<Long, ProductionAsset> assetMap = new HashMap<>();
		...
		while(rs.next()) {
			...
			ProductionAsset asset = assetMap.get(id);
			
			if(asset == null) {
				asset = new ProductionAsset();
				asset.setId(id);
		...
		return new ArrayList<ProductionAsset>(assetMap.values());
	}
}
