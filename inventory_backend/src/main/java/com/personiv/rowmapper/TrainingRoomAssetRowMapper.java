...
public class TrainingRoomAssetRowMapper implements RowMapper<TrainingRoomAsset>{

	@Override
	public TrainingRoomAsset mapRow(ResultSet rs, int arg1) throws SQLException {
		
		TrainingRoomAsset tra = new TrainingRoomAsset();
		...
		Long headsetId = rs.getLong("hsId");
		Long locationId = rs.getLong("locId");
		Long upsId = rs.getLong("uId");
		
		tra.setId(rs.getLong("asId"));
		tra.setCreatedAt(rs.getDate("asCreated"));
		tra.setUpdatedAt(rs.getDate("asUpdated"));
		
		if(locationId != null) {
			TrainingRoom loc = new TrainingRoom();
